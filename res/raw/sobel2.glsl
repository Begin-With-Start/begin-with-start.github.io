...
float colorR(vec2 center,float shiftX,float shiftY){
    return texture2D(uTexture,vec2(textureCoordinate.x+shiftX/uWidth,textureCoordinate.y+shiftY/uHeight)).r;
}

void main(){
    vec2 center=vec2(textureCoordinate.x*uWidth,textureCoordinate.y*uHeight);
